...
%also need to add other parseErrors for rest of error messages
parseErrors(none). %if none error fact then there are no errors
parseErrors(noValidSolution):- %noValidSolution fact in database
	write("No valid solution possible!"), %write output
	nl, %add new line at end
	fail. %stop program
